...
- [Autocomplete và search as you type](#autocomplete-và-search-as-you-type)
  ...
  - [Khi nào nên dùng completion suggester?](#khi-nào-nên-dùng-completion-suggester)
...
- [Debug và đánh giá query](#debug-và-đánh-giá-query)
  ...
  - [Dùng Profile API để tìm nút thắt](#dùng-profile-api-để-tìm-nút-thắt)
...
- [Các lỗi thường gặp](#các-lỗi-thường-gặp)
  - [Dùng sai field `keyword`](#dùng-sai-field-keyword)
  - [Đổi analyzer nhưng không reindex](#đổi-analyzer-nhưng-không-reindex)
  - [Dùng `operator: and` trong `multi_match` sai mục đích](#dùng-operator-and-trong-multi_match-sai-mục-đích)
  - [Bật fuzziness và prefix không kiểm soát](#bật-fuzziness-và-prefix-không-kiểm-soát)
  - [Coi `_score` là sự thật tuyệt đối](#coi-_score-là-sự-thật-tuyệt-đối)
  - [Đưa mọi cú pháp người dùng vào `query_string`](#đưa-mọi-cú-pháp-người-dùng-vào-query_string)
- [Tóm tắt](#tóm-tắt)
...
### Khi nào nên dùng completion suggester?

`search_as_you_type` phù hợp khi cần tìm trong nội dung đã phân tích và muốn kết quả được xếp hạng theo relevance. Nếu mục tiêu là gợi ý một danh sách prefix cố định, chẳng hạn tên sản phẩm hoặc tên địa điểm, `completion` suggester có thể phù hợp hơn. Completion suggester dùng một cấu trúc dữ liệu trong memory để tra prefix nhanh, nhưng cần lập chỉ mục trường `completion` riêng và không thay thế full-text query.
...
PUT product-suggestions/_doc/1
{
  "suggest": {
    "input": ["Elasticsearch", "Elastic Cloud"]
  }
}
...
Hãy chọn `completion` khi cần lookup prefix rất nhanh và dữ liệu gợi ý có cấu trúc. Chọn `search_as_you_type` khi cần tìm trong title hoặc body, hỗ trợ analyzer và trả về document đầy đủ.
...
Validate API với `explain: true` giúp kiểm tra query có hợp lệ và xem Elasticsearch rewrite query như thế nào:
...
### Dùng Profile API để tìm nút thắt

`profile: true` cho biết thời gian Elasticsearch và Lucene dành cho từng phần query trên từng shard. Đây là công cụ chẩn đoán hiệu năng, không phải đồng hồ đo chính xác latency mà người dùng cuối nhìn thấy. Thời gian mạng, queue, fetch phase và việc merge kết quả không được phản ánh đầy đủ trong phần profile.
...
Đọc `profile.shards[].searches[].query[]` để tìm clause có `time_in_nanos` lớn hoặc tạo nhiều `breakdown`. Chạy cùng query nhiều lần trên dữ liệu đại diện vì cache, segment và phân bổ shard có thể làm kết quả thay đổi. Sau khi tìm được nghi vấn, kiểm tra lại bằng benchmark hoặc đo p95 từ phía ứng dụng; không tối ưu chỉ dựa trên một lần profile.

<Callout type="warn" title="Không bật Profile trong production traffic">
  `profile` làm request tốn thêm CPU và trả về payload lớn. Chỉ bật tạm thời trên một request debug, giới hạn `size` và tắt trước khi đưa query trở lại ứng dụng.
</Callout>
