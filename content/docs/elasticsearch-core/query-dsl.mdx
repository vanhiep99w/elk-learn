...
Trước khi viết query, kiểm tra mapping:

```http
GET /products/_mapping
```

Một field `text` được analyzer tách thành token. Một field `keyword` giữ giá trị nguyên vẹn và phù hợp cho exact match, filter, aggregation hoặc sort. Dùng sai loại query thường không phải lỗi JSON; request vẫn chạy nhưng trả về kết quả rỗng hoặc relevance khó đoán.
