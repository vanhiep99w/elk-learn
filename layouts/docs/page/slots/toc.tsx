...
export function TOC({ container, header, footer, style = 'normal', list }: TOCProps) {
  ...
  return (
    <div
      ...
      {...container}
      className={cn(
        'sticky top-(--fd-docs-row-1) h-[calc(var(--fd-docs-height)-var(--fd-docs-row-1))] flex flex-col [grid-area:toc] w-(--fd-toc-width) pt-10 pe-2 pb-2 xl:layout:[--fd-toc-width:268px] max-xl:hidden',
        container?.className,
      )}
    >
      {header}
...
function PageTOCPopoverTrigger({ className, ...props }: ComponentProps<'button'>) {
  ...
  return (
    <CollapsibleTrigger
      className={cn(
        'flex w-full h-10 items-center text-sm text-fd-muted-foreground gap-2 px-3 py-2.5 text-start focus-visible:outline-none [&_svg]:size-4 md:px-4',
        className,
      )}
...
function PageTOCPopoverContent(props: ComponentProps<'div'>) {
  return (
    <CollapsibleContent data-toc-popover-content="" {...props}>
      <div className="flex flex-col px-3 max-h-[50vh] md:px-4">{props.children}</div>
    </CollapsibleContent>
  );
}
